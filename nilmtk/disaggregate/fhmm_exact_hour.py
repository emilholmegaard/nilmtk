...
class FHMM_Hour(object):
    ...
    def train(self, metergroup, **load_kwargs):
        ...
        self.X = dict()
        self.meters = dict()
        new_learnt_models = dict()
        self.individual = dict()
      
        for day_type in self.day_types:
            for t in self.timespan:
                self.meters[t,day_type]=[]
                self.X[t,day_type] = []
                new_learnt_models[t,day_type] = OrderedDict()
                self.individual[t,day_type] = OrderedDict()
                learnt_model[t,day_type]=OrderedDict()    
        ...
        for i, meter in enumerate(metergroup.submeters().meters):
            ...
            for day_type in self.day_types:
                ...
                for t in self.timespan:
                    #try:
                    if True:
                        load_kwargs['preprocessing']=[Clip(), SelectByHourAndDay(hour=t, day=day_type)]
                        #print("Training model for submeter '{}'".format(meter))
                        learnt_model[t,day_type][meter] = hmm.GaussianHMM(2, "full")
                        ...
                        self.X[t,day_type] = X
                        #Put hour and day here
                        try:
                            learnt_model[t,day_type][meter].fit([X])
                        except:
                            print("Error learning model for ln255: '{}'".format(meter))
                                                    # Combining to make a AFHMM
                        
                        
                        try:
                            for meter in learnt_model[t,day_type]:
                                startprob, means, covars, transmat = sort_learnt_parameters(learnt_model[t,day_type][meter].startprob_, learnt_model[t,day_type][meter].means_, learnt_model[t,day_type][meter].covars_, learnt_model[t,day_type][meter].transmat_)
                                new_learnt_models[t,day_type][meter] = hmm.GaussianHMM(startprob.size, "full", startprob, transmat)
                                new_learnt_models[t,day_type][meter].means_ = means
                                new_learnt_models[t,day_type][meter].covars_ = covars
                                # UGLY! But works.
                                self.meters[t,day_type].append(meter)
                        
                            learnt_model_combined = create_combined_hmm(new_learnt_models[t,day_type])
                            self.individual[t,day_type] = new_learnt_models[t,day_type]
                            self.model[t,day_type] = learnt_model_combined
                        
                        except:
                            print("Error learning model for ln268: '{}'".format(meter))
    ...
    def disaggregate_chunk(self, test_mains):
        ...
        """
        learnt_states_array = dict()
        for day_type in self.day_types:
            for t in self.timespan:
                # Array of learnt states
                learnt_states_array[t,day_type] = []
                ...
                learnt_states_array[t,day_type].append(self.model[t,day_type].predict(temp))
        
        
                # Model
                means = OrderedDict()
                for appliance in self.individual[t,day_type]:
                    means[appliance] = self.individual[t,day_type][appliance].means_
                means_copy = deepcopy(means)
                ...
                for learnt_states in learnt_states_array[t,day_type]:
    ...
    def disaggregate(self, mains, output_datastore, **load_kwargs):
        ...
        **load_kwargs : key word arguments
        ...
        output_name = load_kwargs.pop('output_name', 'NILMTK_FHMM_Hour' + date_now)
        ...
        appliances = []
        for i, meter in enumerate(self.meters[1,'B']):
            meter_instance = meter.instance()
