    ...
    def train(self, metergroup):
        ...
        * only uses first chunk for each meter (TODO: handle all chunks).
        ...
        for i, meter in enumerate(metergroup.submeters().meters):
            ...
            for t in self.timespan:
                try:
                    for chunk in meter.power_series(preprocessing=[Clip(), SelectByHour(hour=t)]):
                    ...
                    warn('Problem training the model in hour {}.'.format(t), RuntimeWarning)
    ...
    def disaggregate(self, mains, output_datastore, **load_kwargs):
        ...
        **load_kwargs : key word arguments
        ...
        for t in self.timespan:
            ...
            # Get centroids
            centroids[t] = [m['states'] for m in self.model[t]]
            state_combinations[t] = cartesian(centroids[t])
            ...
            vampire_power_array[t] = np.zeros((n_rows, 1)) + vampire_power
            state_combinations[t] = np.hstack((state_combinations[t], vampire_power_array[t]))
            summed_power_of_each_combination[t] = np.sum(state_combinations[t], axis=1)
            ...
            load_kwargs['preprocessing']=[SelectByHour(hour=t)]
            ...
            #try:
                for chunk in mains.power_series(**load_kwargs):
                    ...
                    indices_of_state_combinations[t], residual_power[t] = find_nearest(
                        summed_power_of_each_combination[t], chunk.values)
                    if printing:
                        print("Hour: {}".format(t))
                        print("MIN and MAX of summed_power_of_each_combination: {} and {}".format(np.min(summed_power_of_each_combination[t]),np.max(summed_power_of_each_combination[t])))
                        print("MIN and MAX of chunk.values: {} and {}".format(np.nanmin(chunk.values),np.nanmax(chunk.values)))
                    
                    
                    for i, m in enumerate(self.model[t]):
                        if printing:
                            print("Estimating power demand for '{}'".format(m['training_metadata']))
                        predicted_power = state_combinations[t][indices_of_state_combinations[t], i].flatten()
        ...
        appliances = []
        #Potential error here by using hour 0
